...
<script>
$(() => {
	// 두 번째 로그인 버튼을 누르면 서버로 비동기 요청한다.
	$("#btnLogin2").click(() => {
		// 요청할 서버의 URL(요청명)
		var s_url = "../realproject/PostLogin.do";
		/* jQuery의 Ajax관련 함수의 파라미터는 JSON형태로 조립한 후 서버로 전송해야 한다.
		이때 <form>태그 하위의 모든 폼값을 한꺼번에 JSON으로 만들고 싶다면 serialize()함수를 사용하면 된다. */
		var s_params = $("#loginFrm").serialize();
		console.log("폼값", s_params);
		// post방식으로 서버로 요청한다. // 콜백함수 정의(콜백된 데이터는 rData로 전달된다.)
		$.post(s_url, s_params, (rData) => {
			/* $.ajax()함수는 콜백데이터의 형식을 지정할 수 있지만,
			$.post()함수는 형식을 지정할 수 없어 무조건 text형식으로만 콜백받을 수 있다.
			따라서 JSON데이터를 콜백받았다면 파싱(Parsing)을 위해 JSON.parse()함수를 먼저 실행한 후 진행해야 한다. */
			var d = JSON.parse(rData);
			if (d.result == 1) {
				// 로그인에 성공한 경우
