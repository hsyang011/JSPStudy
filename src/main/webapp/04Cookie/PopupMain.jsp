<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
/* 해당페이지에서는 가장 먼저 팝업창 오픈에 관련된 쿠키가 있는지 먼저 확인한 후 팝업창을
처리해야 한다. */
// 팝업창 오픈 여부 판단을 위한 변수
String popupMode = "on"; 
...
if (cookies != null) {
    for (Cookie c : cookies) {
        ...
        /* 생성된 쿠키 중에 아래와 같이 쿠키명이 일치하는 쿠키를 찾는다. 조건에 만족한다면
        변수에 쿠키값을 저장한다. */
...
$(() => {
	...
	$("#closeBtn").click(() => {
		// 레이어 팝업창을 숨김처리 한다. (display속성을 none으로 만든다.)
		$("#popup").hide();
		...
		$.ajax({
			...
			// 전송방식(get방식으로 설정)
			type : "get",
			/* 요청시 전송할 파라미터. 여기서는 체크박스의 값을 설정한다. 만약 체크를 했다면
			1, 아니면 null이 전송된다. */
			...
			success : (resData) => {
				...
				/* 만약 콜백데이터가 있다면 페이지를 새로고침한다. F5를 누르는 것과
				동일하다. */
