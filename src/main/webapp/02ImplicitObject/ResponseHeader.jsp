...
*/
// 문자열형식을 날짜형식으로 변형하기 위해 포맷을 설정한다.
/*
응답헤더에 날짜를 지정하는 경우 대한민국은 세계표준시(그리니치 천문대)에 +09
즉 9시간이 빠르므로 그만큼을 더해줘야 정상적인 날짜가 출력된다 만약 09시 이전의 시간으로
세팅되면 하루 전의 날짜가 출력된다.
*/
...
/* <form>태그를 통해 서버로 전송되는 값은 모두 String타입이므로 숫자나 날짜형식의 연산이 필요한
경우에는 변환 후 사용해야 한다. */

// 응답헤더에 값 추가
/*
addDateHeader(헤더명, long타입의 타임스탬프) : 응답헤더에 날짜형식을 추가하는 경우
	long타입의 타임스탬프로 변환 후 추가하면 된다.
...
<body>
	...
	8282가 출력된다. 이것은 getHeader()메소드의 특성으로 처음 입력한 헤더값만
	출력하게 된다.*/
	%>
	
	<h2>myNumber만 출력하기</h2>
	<%
	/* myNumber라는 헤더명으로 2개의 값을 추가했으므로 아래에서는 각각의 값이 정상적으로
	출력된다. 즉 add계열의 메소드는 헤더명을 동일하게 사용하더라도 헤더값은 정상적으로 추가된다.
	즉, 덮어쓰기 되지 않는다. */
