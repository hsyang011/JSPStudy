...
<body>
	...
	<!-- 해당 변수를 통해 확장for문을 실행하낟. items에 지정한 컬렉션에 저장된 객체의
	갯수만큼 반복하여 list로 하나씩 전달한다. -->
	<c:forEach items="${ lists }" var="list">
		<!-- 저장된 순서대로 Person객체를 변수 list에 저장 후 getter를 통해 멤버변수를
		출력한다. -->
	...
	for (String key : keys) {
		...
		/* key와 value를 출력한다. 여기서 value는 Person객체이므로 멤버변수에 저장된 값은
		getter를 통해 출력할 수 있다. */
		out.println("key=>"+key+"<br>");
		out.println("value=>"+p.getName()+","+p.getAge()+"<br>");
	}
	/* JSTL의 확장for문을 사용하면 key를 별도로 얻어와야 하는 번거로움이 없으므로 편리하다. */
	%>
</body>
</html>