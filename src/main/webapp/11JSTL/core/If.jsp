...
속성정리
	...
	result4 : ${ result4 }<br/>
	<!-- test에는 앞뒤에 공백이 하나라도 들어가면 무조건 false를 반환하게된다. 따라서
	주의해야한다. -->
	<c:if test=" ${ true } " var="result5">
		EL양쪽에 빈공백이 있는 경우 false<br />
	</c:if>
	result5 : ${ result5 }<br />
	...
	<!-- 최초실행시에는 파라미터가 없는 상태이므로 아무 내용도 출력되면 안된다. 전송된
	파라미터가 있을때만 실행되도록 한다. -->
	<c:if test="${ not empty param.user }">
		전송된 아이디 : ${ param.user }<br />
		전송된 비번 : ${ param.pass }<br />
		<!-- 아이디, 비번의 일치여부를 판단한 후 결과를 loginResult에 저장한다. if태그는
		else구문이 별도로 없으므로 not을 통해 else와 같은 구문을 구성해야 한다. -->
