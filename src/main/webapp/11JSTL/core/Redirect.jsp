...
<body>
<!-- redirect태그 : 태그가 실행되는 즉시 페이지가 이동이 된다. 새로운 페이지에 대한
	요청이므로 request영역은 공유되지 않는다. 절대경로 사용시 컨텍스트루트 경로는 제외한다. -->
	<!--request영역에 변수를 저장  -->
	...
	<!-- response내장객체를 통한 이동시에는 컨텍스트루트 경로를 반드시 기술해야 한다. 또한
	파라미터가 한글인 경우 깨짐 현상이 발생되므로 URLEncoder클래스를 통해 인코딩 처리를
	해줘야 한다. -->
