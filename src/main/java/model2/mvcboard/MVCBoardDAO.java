...
public class MVCBoardDAO extends JDBConnect {
	...
	/* 게시물의 갯수를 카운트한다. 검색어가 있는 경우 where절을 추가하여 조건에 맞는 게시물을
	카운트한다. */
	public int selectCount(Map<String, Object> map) {
		...
		try {
			...
			/* count(*)는 항상 결과값이 있다. 조건에 맞는 게 없으면 0, 아니면1 1이상의
			값이 항상 반환된다. */
			rs.next();
	...
	// 목록에 출력할 게시물을 얻어오기 위한 메소드
	public List<MVCBoardDTO> selectListPage(Map<String, Object> map) {
		// mvcboard테이블을 대상으로 하므로 타입매개변수 확인 필요함.
		List<MVCBoardDTO> board = new Vector<MVCBoardDTO>();
		
		/* 페이징 처리를 위한 서브쿼리문. 특정 페이지에 해당하는 rownum의 구간으로 게시물을
		인출한다. */
		...
		try {
			/* 인파라미터가 있는 동적쿼리문이므로 prepared객체 생성 및 인파라미터 설정 */
	...
	public int insertWrite(MVCBoardDTO dto) {
		...
		try {
			/* 쿼리문의 일련번호는 모델1 게시판에서 생성한 시퀀스를 그대로 사용한다. 나머지
			값들은 컨트롤러(서블릿)에서 받은 후 모델(DAO)로 전달한다. */
	...
	public boolean confirmPassword(String pass, String idx) {
		...
		try {
			...
			isCorr = false;
			/* 쿼리문 실행 도중 예외가 발생되면 바로 catch절로 넘어오므로 이 경우에도
			false로 값을 설정해야 한다. */
			e.printStackTrace();
		}
		return isCorr;
